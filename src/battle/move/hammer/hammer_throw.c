...
INCLUDE_IMG("battle/move/hammer/poison_hammer.png", battle_move_hammer_throw_poison_hammer_png);
INCLUDE_PAL("battle/move/hammer/poison_hammer.pal", battle_move_hammer_throw_poison_hammer_pal);
...
//#include "battle/move/hammer/poison_hammer.gfx.inc.c"

Gfx battle_move_hammer_throw_poison_hammer_gfx[] = {
    gsDPPipeSync(),
    ...
    gsDPLoadTLUT_pal16(0, battle_move_hammer_throw_poison_hammer_pal),
    gsDPLoadTextureTile_4b(battle_move_hammer_throw_poison_hammer_png, G_IM_FMT_CI, 32, 0, 0, 0, 31, 31, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD),
    gsSPClearGeometryMode(G_LIGHTING),
    gsSPClearGeometryMode(G_SHADING_SMOOTH),
    gsSPVertex(&battle_move_hammer_throw_vtx[0], 4, 0),
...
EntityModelScript N(EMS_PoisonHammer) = STANDARD_ENTITY_MODEL_SCRIPT(N(poison_hammer_gfx), RENDER_MODE_ALPHATEST);
...
API_CALLABLE(N(PickModel)) {
    Bytecode* args = script->ptrReadPos;
    EntityModelScript* model = &N(EMS_BasicHammer);

    switch (gBattleStatus.selectedMoveID) {
        case MOVE_POISON_THROW:
            model = &N(EMS_PoisonHammer);
            break;
        default:
            switch (gBattleStatus.moveArgument)
            {
                case 0:
                    model = &N(EMS_BasicHammer);
                    break;
                case 1:
                    model = &N(EMS_SuperHammer);
                    break;
                case 2:
                    model = &N(EMS_UltraHammer);
                    break;
                default:
                    model = &N(EMS_UltraHammer);
                    break;
    ...
    evt_set_variable(script, *args++, model);
...
API_CALLABLE(N(ApplyDamageBuff)) {
    ...
    {
        case MOVE_POISON_THROW:
            set_next_attack_custom_status(POISON_STATUS, 4, 1, 100);
            break;
    }
...
EvtScript N(EVS_UseMove_Impl) = {
    ...
    EndSwitch
    Wait(3)
    Call(N(PickModel), LVar1)
    Call(CreateVirtualEntity, LVarA, LVar1)
