        ...
        if (!(targetPart->flags & ACTOR_PART_FLAG_DAMAGE_IMMUNE)
            ...
            if (!(target->flags & ACTOR_FLAG_NO_DMG_APPLY)) {
                ...
                if (target->curHP >= 6 && nextHp > 0 && nextHp < 6) {
                    ...
                    if (powerRushes > 0)
                        try_inflict_custom_status(target, target->curPos, CHARGE_STATUS, 1, powerRushes, 100);
                }
                if (target->curHP >= 2 && nextHp > 0 && nextHp < 2) {
                    ...
                    if (megaRushes > 0)
                        try_inflict_custom_status(target, target->curPos, CHARGE_STATUS, 1, megaRushes * 3, 100);
                }
