...
void btl_menu_moves_show_desc(void* data, s32 x, s32 y) {
    Window* window = &gWindows[WIN_BTL_DESC_BOX];
    ...
    switch (MovesMenuState) {
        ...
        case BTL_SUBMENU_STATE_LOCK:
            if (MovesDescVisible) {
                draw_scrollable_desc(MovesOptionDesc[MovesOptionIndexMap[MovesCursorPos]],
                          x+8, y, width, height, MovesTextAlpha, MovesTextColor, 0);
            }
            break;
    }
}
