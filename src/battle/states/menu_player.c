    ...
    switch (gBattleSubState) {
                    ...
                    for (i = 0; i < ARRAY_COUNT(playerData->equippedBadges); i++) {
                        if (playerData->equippedBadges[i] != 0) {
                            ...
                            if (moveData->category == BattleMenu_CategoryForSubmenu[battleStatus->curSubmenu]) {
                                ...
                                DEDUCT_FP_COST(cost, gBattleStatus.playerActor);
        ...
        case BTL_SUBSTATE_STAR_SPIRITS_BUILD_MENU:
            for (i = 0; i < battleStatus->submenuMoveCount; i++) {
                ...
                MovesOptionHudScripts[i] = StarPowerMovesHudScripts[fixed_star_power_index(battleStatus->submenuMoves[i])].enabled;
                if (battleStatus->submenuStatus[i] == 0) {
                    MovesOptionHudScripts[i] = StarPowerMovesHudScripts[fixed_star_power_index(battleStatus->submenuMoves[i])].disabled;
                }
                MovesOptionDisplayCosts[i] = moveData->costFP;
                MovesOptionBPCosts[i] = get_bp_cost_of_move(battleStatus->submenuMoves[i]);
                MovesOptionSortPriority[i] = 0;
