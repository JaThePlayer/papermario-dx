...
#define NAMESPACE temp_atk_down

#define temp_atk_down_DECREMENT_LATE TRUE
...
void N(on_apply)(Actor* actor, Vec3f position, u8 potency) {
    ...
    fx_stat_change(ARROW_TYPE_ATK_DOWN, potency, arrowPos.x, arrowPos.y, arrowPos.z, 1.0f, 60);
...
void N(create_icon)(Actor* target) {
    create_custom_status_icon(target->hudElementDataIndex, ATK_DOWN_ICON_ID);
}

void N(remove_icon)(s32 id) {
    remove_custom_status_icon(id, ATK_DOWN_ICON_ID);
}
